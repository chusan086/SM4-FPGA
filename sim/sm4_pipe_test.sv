...
// 延迟为132个时钟
//////////////////////////////////////////////////////////////////////////////////
module sm4_pipe_test(
    input logic             aclk,           // 时钟信号
    output logic            fan_ctrl                                  
    ); 
    (* mark_debug = "true" *)logic             in_valid;       // 输入数据有效标志                                             
    (* mark_debug = "true" *)logic [127:0]     plaintext;      // 128位明文                                     
    (* mark_debug = "true" *)logic [127:0]     key;            // 128位密钥                                     
    
    (* mark_debug = "true" *)logic            out_valid;      // 输出数据有效标志                                                     
    (* mark_debug = "true" *)logic [127:0]    ciphertext;      // 128位密文   
    vio_0 your_instance_name (
    .clk(aclk),                // input wire clk
    .probe_out0(in_valid),  // output wire [0 : 0] probe_out0
    .probe_out1(plaintext),  // output wire [127 : 0] probe_out1
    .probe_out2(key)  // output wire [127 : 0] probe_out2
    );                                        
